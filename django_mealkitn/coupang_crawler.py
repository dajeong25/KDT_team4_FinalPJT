...
# 쿠팡 페이지 추출 함수
def get_coupang_items(cg_url, user_agt):
...
# 쿠팡 이미지 페이지 추출
def get_coupang_image_page(img_URL, user_agt):
    headers = {"User-Agent":user_agt
               , "Accept-Language": "ko-KR,ko;q=0.8,en-US;q=0.5,en;q=0.3"}
...
# 구성정보 재료 전처리 추가
def p_c_preprocessing_plus(ingredients):
...
# 쿠팡에만 있는 재료만 저장
def ingredient_coupang(data_items_drop):
    # 쿠팡 재료 목록 생성
...
def allergy(dataframe, column, column2):
    allergy_dict = {'갑각류': ['새우', '꽃게', '랍스터', '가재', '대하', '쉬림프', '감바스'],
                    ...
                    '연체동물류': ['조개', '홍합', '굴', '가리비', '바지락', '전복', '꼬막', '해산물', '해물', '문어', '오징어', '낙지', '연포탕',
                              '해신탕',
                              '낙곱새', '성게', '우렁살', '봉골레', '관자'],
                    ...
                    '토마토': ['토마토', '볼로네제'],
                    '생선류': ['굴비', '조기', '고등어', '갈치', '꽁치', '전어', '명태', '노가리', '메기', '동태', '곤이', '아귀', '아구', '코다리',
                            '대구',
...
def dict_to_dataframe(dataframe, column):
    # 요기요 - 쿠팡 밀키트 가격 비교에 사용할 사전
    dict = {'곱창전골': ['곱창전골', '곱창 전골'],
                   '나베': ['나베'],
                   '샤브샤브': ['샤브샤브'],
                   '부대전골': ['부대전골'],
